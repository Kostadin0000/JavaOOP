...
public class ControllerImpl implements Controller {
    ...
    public String startGame() {
        return this.fields.start(this.players.getModels());
    }
    ...
    public String report() {
        List<Player> ct = this.players.getModels()
                .stream()
                .filter(p -> p instanceof CounterTerrorist).collect(Collectors.toCollection(ArrayList::new));


        List<Player> terrorists = (this.players.getModels())
                .stream()
                .filter(p -> p instanceof Terrorist).collect(Collectors.toCollection(ArrayList::new));

        ct = ct.stream()
                .sorted(Comparator.comparingInt(Player::getHealth)
                        .reversed().thenComparing(Comparator.comparing(Player::getUsername)))
        ...
        for (Player playersModel : ct) {
            sb.append(playersModel);
        }
        for (Player playersModel : terrorists) {
            sb.append(playersModel);
        }
